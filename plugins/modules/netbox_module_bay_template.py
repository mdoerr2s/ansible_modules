...
description:
  - Creates, updates or removes module bay templates from NetBox
notes:
  - Tags should be defined as a YAML list
  - This should be ran with connection C(local) and hosts C(localhost)
author:
  - Tobias Groß (@toerb)/Mika Doerr
requirements:
  - pynetbox
version_added: '0.3.0'
...
options:
  data:
    ...
    suboptions:
      device_type:
        description:
          - The device type the module bay template will be associated to. The device type must be "parent".
        required: true
        type: raw
      name:
        description:
      ...
      position:
        description:
          - The position the module bay is associated to
        required: false
        type: str
    type: dict
    required: true
  ...
  tasks:
    - name: Create module bay template within NetBox with only required information
      netbox_module_bay_template:
        ...
        data:
          name: module bay template One
          device_type: Module Type One
        state: present
